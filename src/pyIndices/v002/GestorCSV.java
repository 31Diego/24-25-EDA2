...
public class GestorCSV {
    ...
    public void crearIndice(String nombreColumna) {
        ...
        indices[indiceColumna].ordenar();

        System.out.println("> Índice creado y ordenado para la columna: " + nombreColumna);
    }
    ...
    public void imprimirDatosOrdenadosPor(String nombreColumna) {
        int indiceColumna = obtenerIndiceColumna(nombreColumna);
        if (indiceColumna == -1 || !columnaIndexada[indiceColumna]) {
            System.out.println("La columna no está indexada: " + nombreColumna);
            return;
        ...
        String[] valoresOrdenados = indices[indiceColumna].obtenerTodos();

        for (String valor : valoresOrdenados) {
            int[] posiciones = indices[indiceColumna].buscar(valor);
            for (int fila : posiciones) {
                for (int j = 0; j < columnas; j++) {
                    System.out.printf("%-25.20s", datos[fila][j]);
                }
                System.out.println();
            }
        }
