...
public class Indice {
    ...
    public void ordenar() {
        for (int i = 0; i < cantidadValores - 1; i++) {
            for (int j = 0; j < cantidadValores - 1 - i; j++) {
                if (valores[j].compareTo(valores[j + 1]) > 0) {

                    String tempValor = valores[j];
                    valores[j] = valores[j + 1];
                    valores[j + 1] = tempValor;

                    int[] tempPosiciones = posiciones[j];
                    posiciones[j] = posiciones[j + 1];
                    posiciones[j + 1] = tempPosiciones;

                    int tempContador = contadores[j];
                    contadores[j] = contadores[j + 1];
                    contadores[j + 1] = tempContador;
                }
            }
        }
    }
